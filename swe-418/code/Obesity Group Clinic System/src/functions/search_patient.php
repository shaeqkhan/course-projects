...
require_once("functions/t_config.php");
 ...
 function search_patient($first_name, $last_name, $email, $mobile_number)
 {
	 ...
	 $select_stmt = "SELECT id, CONCAT(first_name,' ',last_name) AS name, email FROM account_details WHERE first_name LIKE '%$first_name%' AND last_name LIKE '%$last_name%' AND email LIKE '%$email%'  AND mobile_number LIKE '%$mobile_number%' AND type_flag = 2";
