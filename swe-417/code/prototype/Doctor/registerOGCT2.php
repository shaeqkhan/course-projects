...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="registerOGCT2.php?code=duplicate";
  $loginUsername = $_POST['username'];
  $LoginRS__query = sprintf("SELECT username FROM account WHERE username=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO account (username, aramco_id, dob, email, fname, lname, gender, password, phone, type_flag) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<div class="contentHolder">
  <p class="contentHolderText">Please fill the form below to register new Healthteam members. (<span class="mandatoryField">*</span>: mandatory field).</p>
  <form class="form" id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
    <h2>Account Details</h2>
    <p>
      <?php
	if(isset($_GET['code'])){
		if($_GET['code']=="duplicate"){
			echo "<p class='error'><img src='../images/x.jpg' width='20' height='20' /> Registration failed: User already exist!</p>";
		}
		else{
